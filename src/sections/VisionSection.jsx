...
const ScrollingMarquee = memo(({ direction, color, baseOffset, scrollEnd, verticalOffset }) => {
...
const VisionSection = () => {
    ...
    () =>
      Array.from({ length: 6 }, (_, index) => {
        ...
        return {
          ...
          scrollEnd: direction === "left" ? randomBetween(60, 160) : randomBetween(-260, -120),
  ...
  return (
    <div className="dual-main">
      <motion.div className="dual-left" variants={fadeIn} initial="hidden" whileInView="visible">
        <MediaImage src={assetImages.ui.candle} alt="Studio still life" />
      </motion.div>

      <motion.div className="dual-right" variants={fadeIn} initial="hidden" whileInView="visible">
        <div className="container-fancy">
          <motion.h1 className="title-fancy" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
            VISION
          </motion.h1>
          <motion.h3 className="description-fancy" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }}>
            LESS NOISE, MORE IMPACT. MELLO YELLOW BELIEVES IN THE POWER OF CLEAR MESSAGING, STUNNING VISUALS, AND SEAMLESS EXECUTION.
            WE STRIP AWAY THE UNNECESSARY TO LET BOLD IDEAS SHINE.
          </motion.h3>
          <div className="content-wrapper-fancy">
            ...
            <div className="overlay-image">
              <AnimatePresence mode="wait">
                <ImageCarousel images={galleryImages} minDelay={3000} maxDelay={5000} />
              </AnimatePresence>
            </div>
          </div>
        </div>
